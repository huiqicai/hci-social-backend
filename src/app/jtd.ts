 ...
 *                    !!! NOTE !!!
 * 
 * This is pulled from https://github.com/ajv-validator/ajv/blob/418cd0f4308c07c627071b4d03544e7cc57f235c/lib/types/jtd-schema.ts
 * (MIT license)
 * 
 * Foal 2.x uses AJV 6, which does not actually support JTD (only JSON Schema) so this is technically incorrect.
 * However, it is reasonably close and gets us closer to having reasonable type safety, so I'm using it anyways.
 * If I encounter any concrete cases where it's incorrect and I can adjust it to be more accurate to what we have,
 * I'll make changes and make a note of it here.
 * 
 * * 'number' added to NumberType
 * * ref changed to $ref
 * * elements changed to items
 * 
 ******************************************************/

/** numeric strings */
type NumberType = 'float32' | 'float64' | 'int8' | 'uint8' | 'int16' | 'uint16' | 'int32' | 'uint32' | 'number'
